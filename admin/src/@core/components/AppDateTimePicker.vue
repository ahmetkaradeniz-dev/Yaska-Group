...
locale.value = localStorage.getItem('app-language') || 'tr'

const config = ref({
  ...compAttrs.config,
  locale: localStorage.getItem('app-language') == 'tr' ? Turkish : english,
  altFormat: localStorage.getItem('app-language') == 'tr' ? 'j F Y' : 'F j, Y',
})

fieldProps.label = t(compAttrs.config.i18n)

watch(locale, () => {
  config.value.locale = locale.value === 'en' ? english : Turkish
  config.value.altFormat = localStorage.getItem('app-language') == 'tr' ? 'j F Y' : 'F j, Y',
    fieldProps.label = t('reusable.' + compAttrs.config.i18n)
})
</script> 
...
<template>
  ...
  <VInput v-bind="{ ...inputProps, ...rootAttrs }" :model-value="modelValue" :hide-details="props.hideDetails"
    class="position-relative" :rules="props.rules">
    <template #default="{ isDirty, isValid, isReadonly }">
      <!-- v-field -->
      <VField v-bind="fieldProps" :active="focused || isDirty.value || isCalendarOpen"
        :focused="focused || isCalendarOpen" role="textbox" :dirty="isDirty.value || props.dirty"
        :error="isValid.value === false" @click:clear="onClear">
        <template #default="{ props: vFieldProps }">
          <div v-bind="vFieldProps">
            <!-- flat-picker  -->
            <FlatPickr v-if="!isInlinePicker" v-bind="compAttrs" ref="refFlatPicker" :model-value="modelValue"
              class="flat-picker-custom-style" :disabled="isReadonly.value" @on-open="isCalendarOpen = true"
              @on-close="isCalendarOpen = false" @update:model-value="emitModelValue" :config="config" />
            <!-- simple input for inline prop -->
            <input v-if="isInlinePicker" :value="modelValue" class="flat-picker-custom-style" type="text">
          </div>
        </template>
      </VField>
    </template>
